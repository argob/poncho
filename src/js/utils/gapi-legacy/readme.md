...
Convierte una respuesta de la Google Sheets API v4 a la estructura
legacy de la versión 3, para mantener compatibilidad con integraciones
que aún esperan el formato `feed/entry`.

## Problema que resuelve

La API v3 de Google Sheets (deprecada) devolvía los datos en una
estructura con `feed` y `entry`, donde cada columna se representaba
como `gsx$<nombre>` con su valor bajo la clave `$t`. La API v4
actual devuelve un array de arrays bajo la propiedad `values`.

Esta función actúa como adaptador: recibe la respuesta de la v4 y
devuelve un objeto con la forma exacta que esperaba la v3.
