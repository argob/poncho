...
const pmOptions = {
    ...
    title: "name",
    tooltip: true,
    headers: headers, // @see Documentación para headers
    template_structure: {
        values: [
            "address",
            "locality",
...
const pm = new PonchoMapFilter(demoData, pmOptions);
pm.render();

const pmSearchOptions = {
    scope: "search-poncho-map",
    placeholder: "Buscar por provincia, localidady y dirección",
    combobox: true,
    search_fields: [
        "address",
        "locality",
...
const pmSearch = new PonchoMapSearch(pm, pmSearchOptions);
pmSearch.render();
</script>
<!-- / SCRIPTS -->
```