...
function ponchoTable(opt) {
  ...
  function getSheetValues(url) {
    jQuery.getJSON(url, function (data) {
      ...
      jQuery("#ponchoTable caption").html(opt.tituloTabla); //Agregar titulos al thead de la tabla

      jQuery('#ponchoTable thead tr').empty();
      jQuery('#ponchoTable thead tr').append(titulos); //CONTENIDO FILAS
      ...
      jQuery.each(listado, function (row, value) {
        if (row > 1) {
          ...
          jQuery.each(filteredTitle, function (index, title) {
            var tdEmpty = '';
            var filas = listado[row][index]; //Detectar si es botón
            ...
            if (!filas) {
              filas = ''; //Ocultar filas vacias en mobile

              tdEmpty = 'hidden-xs';
            }

            concatenado += filas; //Aplicar markdown a todas las filas
  ...
  function initDataTable() {
    function removeAccents(data) {
      return data.replace(/έ/g, 'ε').replace(/[ύϋΰ]/g, 'υ').replace(/ό/g, 'ο').replace(/ώ/g, 'ω').replace(/ά/g, 'α').replace(/[ίϊΐ]/g, 'ι').replace(/ή/g, 'η').replace(/\n/g, ' ').replace(/[áÁ]/g, 'a').replace(/[éÉ]/g, 'e').replace(/[íÍ]/g, 'i').replace(/[óÓ]/g, 'o').replace(/[úÚ]/g, 'u').replace(/ê/g, 'e').replace(/î/g, 'i').replace(/ô/g, 'o').replace(/è/g, 'e').replace(/ï/g, 'i').replace(/ü/g, 'u').replace(/ã/g, 'a').replace(/õ/g, 'o').replace(/ç/g, 'c').replace(/ì/g, 'i');
    }
    ...
    var tabla = jQuery("#ponchoTable").DataTable({
      ...
      "dom": "<'row'<'col-sm-6'l><'col-sm-6'f>>" + "<'row'<'col-sm-12'i>>" + "<'row'<'col-sm-12'tr>>" + "<'row'<'col-md-offset-3 col-md-6 col-sm-offset-2 col-sm-8'p>>",
      "language": {
    ...
    jQuery(document).ready(function () {
      ...
      jQuery('#ponchoTableSearch').keyup(function () {
        tabla.search(jQuery.fn.DataTable.ext.type.search.string(this.value)).draw();
      });
    }); //BUSCADOR

    jQuery("#ponchoTable_filter").parent().parent().remove(); //FILTRO PERSONALIZADO
    ...
    jQuery('#ponchoTableFiltro').on('change', function () {
      ...
      if (filtro != "") {
        tabla.column(filtroColumna).every(function () {
          var that = this;
          that.search(filtro ? '^' + filtro + '$' : '', true, false).draw();
        });
      } else {
...
function ponchoMaps(opt) {
  ...
  var map = new google.maps.Map(document.getElementById("map"), mapOptions); //TRAER DATOS

  jQuery.getJSON('https://spreadsheets.google.com/feeds/list/' + opt.idSpreadheet + '/' + opt.hojaNumero + '/public/values?alt=json', function (data) {
    ...
    var markers = [];
    var detalles = []; //Template del Icono marker map

    var markerPath = "M206.549,20.359L206.549,20.359c-74.459,0-134.584,60.126-134.584,134.584c0,25.961,8.293,50.751,19.832,71.122" + "l87.71,151.801c5.499,9.916,16.586,14.874,27.043,14.874c10.458,0,21.003-4.958,27.043-14.874l87.71-151.711" + "c11.629-20.373,19.832-44.712,19.832-71.124C341.133,80.574,281.008,20.359,206.549,20.359z M206.549,206.978" + "c-33.804,0-61.41-27.606-61.41-61.41s27.606-61.41,61.41-61.41s61.41,27.606,61.41,61.41" + "C267.959,179.484,240.353,206.978,206.549,206.978z";
    var template = ['<?xml version="1.0"?>', '<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="286.818px" height="396.798px" viewBox="63.14 8.15 286.818 396.798" enable-background="new 63.14 8.15 286.818 396.798" xml:space="preserve">', '<path stroke="#FFF" stroke-width="10" fill="{{ color }}" d="' + markerPath + '"/>', '</svg>'].join('\n');
    var colores = {
      "azul": {
        "hex": "#0072BB"
      },
    ...
    jQuery.each(puntos, function (key, punto) {
      var color = punto.gsx$color.$t.toLowerCase(); //Cambio color de marker segun el tipo de punto

      var svg = template.replace('{{ color }}', colores[color]["hex"]);
      var latLng = new google.maps.LatLng(punto.gsx$latitud.$t, punto.gsx$longitud.$t); //Tipo de punto
      ...
      if (punto.gsx$telefono.$t != "") {
        telefono = "<div><i class='fa fa-phone'></i>" + punto.gsx$telefono.$t + "</div>";
      }

      if (punto.gsx$email.$t != "") {
        email = "<div><i class='fa fa-envelope'></i>" + punto.gsx$email.$t + "</div>";
      }
      ...
      if (punto.gsx$boton.$t != "") {
        boton = "<hr><a class='btn btn-success btn-sm' href=" + punto.gsx$boton.$t + ">" + opt.textoBoton + "</a>";
      }

      if (punto.gsx$foto != null) {
        foto = '<img width="100%" src="' + punto.gsx$foto.$t + '">';
      } //DETALLE CADA PUNTO
...
var ponchoUbicacion = function ponchoUbicacion(options) {
  ...
  function getDropDownList(name, id, optionList) {
    ...
    var emptyOption = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
    var selected_item = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : false;
    ...
    jQuery.each(optionList, function (i, el) {
      ...
      combo.append("<option value='" + el.id + "' " + selected + ">" + el.nombre + "</option>");
    });
    return combo;
  }
  ...
  function getSelectProvincias(provincias) {
    ...
    var select = getDropDownList('sProvincias', 'sProvincias', provinciasOptions, required, true, iProvincia.val());
    return select;
  }
  ...
  function getSelectLocalidades(localidades, provincia) {
    ...
    var select = getDropDownList('sLocalidades', 'sLocalidades', [], required, true, false);

    if (iProvincia.val()) {
      locaSelect = localidades.filter(function (localidad) {
        return String(localidad.provincia.id) == String(provincia);
      }).map(function (a) {
        if (a.departamento.nombre) {
          a.nombre = capitalizeFirstLetter(a.departamento.nombre.toLowerCase()) + ' - ' + capitalizeFirstLetter(a.nombre.toLowerCase());
        }
      ...
      select = getDropDownList('sLocalidades', 'sLocalidades', locaSelect, required, true, iLocalidad.val());
    }
...
function ponchoChart(opt) {
  ...
  if (chequeoOpcionesObligatorias(opt)) {
    if (opt.jsonInput) {
      ...
      var url = opt.jsonUrl ? opt.jsonUrl : 'https://sheets.googleapis.com/v4/spreadsheets/' + opt.idSpread + '/values/' + opt.hojaNombre + '?alt=json&key=AIzaSyCq2wEEKL9-6RmX-TkW23qJsrmnFHFf5tY';
      jQuery.getJSON(url, function (data) {
        drawChart(data, opt);
      });
    }
  ...
  function graficoTorta(etiquetas, datos, tipoGrafico, colores, idGrafico, posicionLeyendas, toltips, mostrarLeyendas) {
    var $grafica = document.getElementById(idGrafico);
    var dataset = {
      data: datos,
      borderColor: colores,
  ...
  function graficoComplejoStacked(etiquetas, tipoGrafico, datos, idGrafico, empiezaYenCero, posicionLeyendas, toltips, mostrarLeyendas) {
    //Stacked Bar
    var $grafica = document.getElementById(idGrafico);
    new Chart($grafica, {
      type: tipoGrafico,
        ...
        tooltips: {
          ...
          callbacks: {
            label: function label(tooltipItems, data) {
              var text = '';
              if (indice == 2) text = '%';else if (tooltipItems.datasetIndex == indice) text = '%';
  ...
  function graficoHeatMap(etiquetas, datos, idGrafico, labelsY, rango, labelX, labelY, labelValor, titulo, mostrarYaxis, posicionLeyendas, mostrarLeyendas) {
    ...
    var options = {
      ...
      tooltip: {
        custom: function custom(_ref) {
          ...
          return '<div class="arrow_box">' + '<span>' + labelX + ": " + labelsY[seriesIndex] + '<br>' + labelY + ": " + w.globals.labels[dataPointIndex] + '<br>' + labelValor + ": " + value + '</span>' + '</div>';
        }
      },
      plotOptions: {
        heatmap: {
    ...
    chart.render();
    var collection = document.getElementsByClassName("apexcharts-toolbar");
  ...
  function chequeoOpcionesObligatorias(opt) {
    var chequeo = false;
    if ((opt.idSpread && opt.hojaNombre || opt.jsonUrl || opt.jsonInput) && typeof opt.tipoGrafico != 'undefined' && opt.tipoGrafico != "" && typeof opt.idComponenteGrafico != 'undefined' && opt.idComponenteGrafico != "" && getTipoGrafico(opt.tipoGrafico) != "") chequeo = true;
    return chequeo;
  }

  function numeroFormateado(numero) {
    var value = numero.toString().replace('.', ',');
    var array = value.split(","); //var result1 = new Intl.NumberFormat('es').format(array[0]);

    var result1 = new Intl.NumberFormat('es-AR', {
      maximumFractionDigits: 2
    }).format(array[0]);
    if (array.length > 1) value = result1.concat(",", array[1].substr(0, 2));else value = result1;
    return value;
  }
  ...
  function drawChart(data, opt) {
    ...
    var listado = data['values']; //TITULOS
    ...
    jQuery.each(listado, function (row, value) {
      if (row == 0) {
        //construyo arrays para los dataset, recupero colores y labels
      ...
      if (row > 1) {
        //recupero los datos para los dataset y los colores para torta
        var label = false;
        jQuery.each(filteredTitlePos, function (index, title) {
          ...
          toltips = {
            ...
            callbacks: {
              ...
              footer: function footer(tooltipItems, data) {
                var total = tooltipItems.reduce(function (a, e) {
                  return a + parseFloat(e.yLabel);
                }, 0);
                return 'Total: ' + new Intl.NumberFormat('es-AR', {
                  maximumFractionDigits: 2
                }).format(total) + '%';
              }
            }
    ...
    if (cantDatos > 1) {
      if (tipoGrafico == 'heatmap') {
        if (typeof opt.heatMapColors != 'undefined' && opt.heatMapColors != "" && typeof opt.heatMapColorsRange != 'undefined' && opt.heatMapColorsRange != "") {
          ...
          var rango = []; //construyo range object
        ...
        var datasets = [];
        var indiceColor = 0; //getColores

        colores.forEach(function (valor, indice, array) {
          codigosColores.push(getColor(valor));
        });
        console.log('colores --> ' + codigosColores);
        var indiceMixed = 0; //getDatos
        ...
        jQuery.each(Object.keys(filteredTitleName), function (index, key) {
          ...
          if (valores.hasOwnProperty(pos)) {
            ...
            if (opt.tipoGrafico == 'Line') {
              ...
              var tipo = tipoGraficosMixed[indiceMixed]; //construyo datasets
        ...
        if (opt.tipoGrafico == 'Stacked Bar') graficoComplejoStacked(etiquetas, tipoGrafico, datasets, opt.idComponenteGrafico, opt.ejeYenCero, posicionLeyendas, toltips, mostrarLeyendas);else if (opt.tipoGrafico == 'Mixed') graficoComplejoMixed(etiquetas, 'bar', datasets, opt.idComponenteGrafico, opt.ejeYenCero, posicionLeyendas, indicePorcentajeMixed, columnas[0], columnas[1], mostrarLeyendas);else if (opt.tipoGrafico == 'Horizontal Bar') graficoComplejoHorizontal(etiquetas, tipoGrafico, datasets, opt.idComponenteGrafico, opt.ejeYenCero, posicionLeyendas, toltips, mostrarLeyendas);else graficoComplejo(etiquetas, tipoGrafico, datasets, opt.idComponenteGrafico, opt.ejeYenCero, posicionLeyendas, toltips, mostrarLeyendas);
      }
...
function gapi_legacy(response) {
  ...
  var regex = / |\/|_/ig;
  var entry = [];
  response.values.forEach(function (v, k) {
    if (k > 0) {
      var zip = {};

      for (var i in keys) {
        var d = v.hasOwnProperty(i) ? v[i].trim() : "";
        zip["gsx$".concat(keys[i].toLowerCase().replace(regex, ''))] = {
          "$t": d
        };
      }
